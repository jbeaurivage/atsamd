...
/// Marker trait representing a pin which can be used as an I2C pad
pub trait IsI2c: Sealed {}
    ...
    SCL: SomePad,
{
    ...
    #[inline]
    pub fn new(sda: impl IsPad, scl: impl IsPad) {
        Self {
            sercom: PhantomData,
            sda,
            scl,
    ...
    SCL: SomePad,
{
    /// Create a new [`Pads`] struct. `SDA` must always be SERCOM pad 0, and `SCL` SERCOM pad 1.
    #[inline]
    pub fn new<ISDA, ISCL>(
        sda: impl AnyPin<Id = SDA>,
        scl: impl AnyPin<Id = SCL>,
    ) -> Pads<S, Pad<S, ISDA>, Pad<S, ISCL>> {
        Self {
            sercom: PhantomData,
...
/// - Specify `SDA` as Pad 0 and `SCL` as Pad 1;
/// - Use a valid combination of I2C pads (refer to datasheet, I/O Multiplexing and Considerations)
pub trait ValidPads: PadSet {}

impl<S, SDA, SCL> ValidPads for Pads<S, SDA, SCL>
where
    S: Sercom,
    SDA: IsPad<PadNum = Pad0>,
    SCL: IsPad<PadNum = Pad1>,
{
}
